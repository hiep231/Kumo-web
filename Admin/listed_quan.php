...
<body>
    ...
    <div class="rightside">
        <div class="container mt-3">  <!-- class="container" -->
            <form  id="searchForm" method="post" action="listed_quan.php">
            ...
            <?php
                $sql = "SELECT * FROM product WHERE category = 'Quần'";
                ...
                if (isset($searchValue)) {
                    // $startingLimit = 0;
                    $lietke_sql = "SELECT product.id,name,product.price,images,specName,brand,promotion.price as pro_price,describes,ratingStar FROM product JOIN promotion ON product.salePromotion  = promotion.id WHERE product.name LIKE ? AND category = 'Quần' LIMIT $startingLimit, $elementOfPages";
                    // var_dump($lietke_sql);
                    ...
                    $count_sql = "SELECT COUNT(*) as count FROM product WHERE name LIKE ? and category = 'Quần'";
                    ...
                    $lietke_sql = "SELECT product.id,name,product.price,images,specName,brand,promotion.price as pro_price,describes,ratingStar FROM product JOIN promotion ON product.salePromotion  = promotion.id WHERE category = 'Quần' LIMIT $startingLimit, $elementOfPages";
            ...
            <button class="btn btn-color my-1 p-1" id="prevPage">
                ...
                for ($btn = 1; $btn <= $totalPages; $btn++) {
                    echo '<button class="btn btn-color mx-md-1 my-1 p-1 page-' . $btn . '"><a href="listed_quan.php?page=' . $btn . '&search=' . (isset($_GET['search']) ? urlencode($_GET['search']) : '') . '" class="text-dark fw-bold text-decoration-none py-1 px-3">' . $btn . '</a></button>';
                }
            ?>
            <button class="btn btn-color my-1 p-1" id="nextPage">
                <?php if ($page < $totalPages) : ?>
                    <a href="listed_quan.php?page=<?php echo ($page + 1); ?>&search=<?php echo (isset($_GET['search']) ? urlencode($_GET['search']) : ''); ?>" class="text-dark fw-bold text-decoration-none py-1 px-2"><svg xmlns="http://www.w3.org/2000/svg" width="20" height="17" viewBox="0 0 20 20"><path fill="#00000" d="M7 1L5.6 2.5L13 10l-7.4 7.5L7 19l9-9z"/></svg></a>
                    <?php else :?>
    ...
    <script>
        ...
        // function processProduct(button) {
        //     var id = button.getAttribute("data-id"); 
        //     console.log("Xử lý sản phẩm với ID:", id);
        ...
        //     window.location.href = 'listed_quan.php?page=' + page;
        // }
        ...
        function submitForm() {
            ...
            var formAction = 'listed_quan.php?search=' + encodeURIComponent(searchInputValue);
