...
if(isset($_POST['submit'])){
    ...
    // echo $code;
    if ($checkcode == $code){
        $insert = $conn->prepare("INSERT INTO `users` (phone, email, userName, passwords, avatar,gender,addressesList,isAdmin,status) VALUES ('',?, ?, ?, ?, '', '', ?, 'Hoạt động')");
        $insert->execute([$email, $userName, md5($passwords), 'man-user.png', $isAdmin]);
        // $update = "UPDATE users SET status = 'Hoạt động' WHERE email = $email";
        // $result = mysqli_query($conn, $update);
        header('Location: ../Admin/listed_account.php?success=1');
    }else {
...
<div class="form-container">
    <a href="../Admin/listed_account.php" class="dialog-close-btn-register-sendmail"><svg xmlns="http://www.w3.org/2000/svg" width="40" height="40" viewBox="0 0 24 24"><path fill="#888888" d="m12 12.708l-5.246 5.246q-.14.14-.344.15q-.204.01-.364-.15t-.16-.354q0-.194.16-.354L11.292 12L6.046 6.754q-.14-.14-.15-.344q-.01-.204.15-.364t.354-.16q.194 0 .354.16L12 11.292l5.246-5.246q.14-.14.344-.15q.204-.01.364.15t.16.354q0 .194-.16.354L12.708 12l5.246 5.246q.14.14.15.344q.01.204-.15.364t-.354.16q-.194 0-.354-.16L12 12.708Z"/></svg></a>
